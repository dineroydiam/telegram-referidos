...
def mover_usuario(user):
    try:
        ...
        if not user_id:
            print(f"⚠️ ERROR: No se encontró el ID de {user}. Es posible que el usuario no haya interactuado con el bot.")
            bot.send_message(GROUP_ESPANOL_ID, f"⚠️ {user}, tu ID no está registrado. Envíame un mensaje en privado con 'PARTICIPAR' para que pueda agregarte.")
            return
